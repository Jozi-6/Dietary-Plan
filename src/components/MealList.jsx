...
const MealList = ({ meals, onUpdateMeal }) => {
  ...
  const getFeelingsColor = (feeling) => {
    const colors = {
      ...
      'Energized': 'bg-green-100 text-green-700',
      'Heavy': 'bg-orange-100 text-orange-700',
      'Reflux': 'bg-purple-100 text-purple-700',
      'Normal': 'bg-blue-100 text-blue-700',
      'Gassy': 'bg-yellow-100 text-yellow-700'
    }
    return colors[feeling] || 'bg-gray-100 text-gray-700'
  }
  ...
  if (meals.length === 0) {
    return (
      <div className="p-12 text-center">
        ...
        <h3 className="text-lg font-medium text-gray-600 mb-2">No meals logged today</h3>
        <p className="text-gray-400">Start logging your intake to see your timeline here</p>
      </div>
    )
  }
  ...
  return (
    <div className="divide-y divide-gray-100">
      {meals.map((meal, index) => {
        ...
        return (
          <div key={meal.id} className="p-4 hover:bg-gray-50 transition-all">
            <div className="flex items-start gap-4">
              {/* Timeline Line */}
              {index !== meals.length - 1 && (
                <div className="absolute left-8 mt-12 w-0.5 h-full bg-emerald-200" />
              )}

              {/* Safety Indicator */}
              <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
                safety.color === 'green' ? 'bg-green-100' :
                safety.color === 'red' ? 'bg-red-100' :
                'bg-amber-100'
              }`}>
                <SafetyIcon className={`w-5 h-5 ${
                  safety.color === 'green' ? 'text-green-600' :
                  safety.color === 'red' ? 'text-red-600' :
                  'text-amber-600'
                }`} />
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-semibold text-gray-800">{meal.item}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          safety.color === 'red' ? 'bg-red-100 text-red-700' :
